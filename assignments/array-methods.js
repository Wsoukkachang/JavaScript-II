...
runners.forEach(function (item, index){
    let full = (`${item.first_name} ${item.last_name}`);
    fullName[index] = full;
})
...
let largeShirts = runners.filter(filterSize);
...
// Problem 1 --- Send email to runners who donated more than $100 (will also send free shirt)

function filterDonations(item) {
    return item.donation > 100;
    }

//Filter out high donors from list
let highDonors = runners.filter(filterDonations);

//List full name, email, shirt size, and donation amount of high donors
let listHighDonors = highDonors.map( function(item, index) {
...
console.log('Here are your high donors:', listHighDonors);


// Problem 2 --- Using previous scenario, find how many of each size we need to order

// Arrays with each shirt size
let small = [];
let medium = [];
let large = [];
let xl = [];

listHighDonors.forEach(function (item, index){
    
    let fullName = (`${item.full_name}`);
    if (item.shirt_size === 'S' || item.shirt_size === 'XS'){
        small.push(`Name: ${fullName}: Email: ${item.email}, Shirt Size: ${item.shirt_size}`) ;
    }
    else if (item.shirt_size === 'M'){
        medium.push(`Name: ${fullName}: Email: ${item.email}, Shirt Size: ${item.shirt_size}`) ;
    }
    else if (item.shirt_size === 'L'){
        large.push(`Name: ${fullName}: Email: ${item.email}, Shirt Size: ${item.shirt_size}`) ;
    }
    else if (item.shirt_size === 'XL',"2XL","3XL") {
        xl.push(`Name: ${fullName}: Email: ${item.email}, Shirt Size: ${item.shirt_size}`) ;
    }
})


// Function that prints how many shirts are needed to order and list of runners
function howMany(array) {
console.log(`You need to order ${array.length} shirts.`);
console.log(array);
}

console.log(howMany(small));
console.log(howMany(medium));
console.log(howMany(large));;
console.log(howMany(xl));

// Problem 3 --- Find out which company had the most employees that donated (not donantion amount)(Skinix --- BTW)
...
// Group runners by companies
let groupedPeople = groupBy(runners, 'company_name');
